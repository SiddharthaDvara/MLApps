...
if uploaded_files :
    ...
    if Outlier:
        for t in df.columns:
            if df[t].dtypes in ["int64","float64"]:
                m=["Mean","Median"]
                menu = st.sidebar.selectbox("Select the option to be imputed for outlier treatment ", m,key="Outlier")
    ...
    if Missing_value:
        for t in df.columns:
            if df[t].dtypes in ["int64","float64"]:
                ...
                menu = st.sidebar.selectbox("Select the option to be imputed for Missing value treatment", l,key="Missing value")
                if menu == "Mean":
            ...
            elif df[t].dtypes in  ["object","str"]:
        ...
        for t in df.columns:
            ...
            if df[t].dtypes  in ["int64","float64"]:
                ...
                if null_sum > 0:
                    ...
                    options = st.sidebar.selectbox("Select feature scaling methods",['Standard Scalar', 'Min Max Scalar', 'Robust Scalar','Max Absolute scalar'],key="Feature Scaling")
    ...
    if export:
        ...
        options=st.sidebar.selectbox("Enter the file to be exported",['csv','xlsx'],key="Export")
